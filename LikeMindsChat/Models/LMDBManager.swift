...
class LMDBManager {
  ...
  ///   - Schema version set to 8
  ///   - Migration block to handle schema updates
  /// - Note: Handles migrations for schema versions up to 8, including updates to MemberRO, UserRO, ConversationRO, LastConversationRO, WidgetRO, and AttachmentRO
  static private func dbConfig() -> Realm.Configuration {
    ...
    config.schemaVersion = 8  // Increment schema version
    config.migrationBlock = { (migration, oldSchemaVersion) in
      ...
      if oldSchemaVersion <= 7 {
        migration.enumerateObjects(ofType: WidgetRO.className()) { oldObject, newObject in
          // Create new LMMetaRO object if old _lm_meta exists and contains valid data
          if let oldLMMeta = oldObject?["_lm_meta"] as? Data {
            do {
              // Try to deserialize the old Data to dictionary
              if let dict = try JSONSerialization.jsonObject(with: oldLMMeta) as? [String: Any] {
                // Create new LMMetaRO object
                let lmMetaRO = migration.create(LMMetaRO.className())

                // Map old values to new LMMetaRO object
                lmMetaRO["sourceChatroomId"] = dict["source_chatroom_id"] as? String
                lmMetaRO["sourceChatroomName"] = dict["source_chatroom_name"] as? String
                lmMetaRO["type"] = dict["type"] as? String

                // Assign the new LMMetaRO object to the widget
                newObject?["lmMeta"] = lmMetaRO
              }
            } catch {
              print("Migration error: Failed to convert old _lmMeta data: \(error)")
              newObject?["lmMeta"] = nil
            }
          } else {
            newObject?["lmMeta"] = nil
          }
