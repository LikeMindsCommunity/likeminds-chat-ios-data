...
class PollClient {
  ...
  func postPollConversation(
    request: PostPollConversationRequest, response: LMClientResponse<PostPollConversationResponse>?
  ) {
   ...
   */
  func addPollOption(
    request: AddPollOptionRequest, response: LMClientResponse<AddPollOptionResponse>?
  ) {
    ...
    DataNetwork.shared.requestWithDecoded(
      ...
      PollDBService.shared.updateNewPollOption(
        poll: data.data?.poll, conversationId: request.conversationId)
      response?(data)
    } failureCallback: { (moduleName, error) in
      response?(LMResponse.failureResponse(error.localizedDescription))
    }
  ...
  func submitPoll(request: SubmitPollRequest, response: LMClientResponse<NoData>?) {
    ...
    DataNetwork.shared.requestWithDecoded(
      ...
      if data.success {
        PollDBService.shared.updateConversationSubmitPolls(
          conversationId: request.conversationId, polls: request.polls)
        ConversationClient.shared.loadConversation(
          withConversationId: request.conversationId, chatroomId: request.chatroomId)
      }
      response?(data)
    } failureCallback: { (moduleName, error) in
      response?(LMResponse.failureResponse(error.localizedDescription))
