...
class ChatroomClient: ServiceRequest {
  ...
  /**
   * Converts client request model to internal model and calls the api
   * @param markReadChatroomRequest - client request model to mark a chatroom as read
   * @throws IllegalArgumentException - when LMChatClient is not instantiated or required properties not provided
   * @return LMResponse<Nothing> - Base LM response
   */
  ...
  func getChannelInvites(
    ...
    let networkPath = ServiceAPIRequest.NetworkPath.getChannelInvites(request)
    let endpoint = networkPath.apiURL
    guard let url = endpoint.url else {
      response?(LMResponse.failureResponse("Invalid URL"))
      return
  ...
  func updateChannelInvite(
    ...
    let networkPath = ServiceAPIRequest.NetworkPath.updateChannelInvite(request)
    let endpoint = networkPath.apiURL
    guard let url = endpoint.url else {
      response?(LMResponse.failureResponse("Invalid URL"))
      return
