...
class ConversationDBService {
    ...
    func getAboveConversations(
        ...
        return conversations.where { query in
            query.chatroomId == chatroomId
                && !query.state.in(filterConversations)
                && query.createdEpoch < timestmap
        }
    ...
    func getChatroomConversations(
        chatroomId: String, filterConversations: [Int] = []
    ) -> Results<ConversationRO>? {
        let realm = LMDBManager.lmDBInstance()
        let conversations = ChatDBUtil.shared.getChatroomConversations(
            realm: realm, chatroomId: chatroomId,
            filterConversations: filterConversations)
        return conversations
    }

    func deleteConversationPermanently(
        conversationId: String, chatroomId: String
    ) {
        let realm = LMDBManager.lmDBInstance()
        guard
            let chatroom = ChatDBUtil.shared.getChatroom(
                realm: realm, chatroomId: chatroomId)
        else { return }

        guard
            let conversation = ChatDBUtil.shared.getConversation(
                realm: realm, conversationId: conversationId)
        else { return }
    ...
    func updateConversation(conversation: _Conversation_) {
        let memberRO = ROConverter.convertMember(
            member: conversation.member,
            communityId: SDKPreferences.shared.getCommunityId() ?? conversation
                .communityId ?? "")
        guard
            let conversationRO = ROConverter.convertConversation(
                conversation: conversation, member: memberRO)
        else { return }
    ...
    func updateConversationUploadingStatus(
        conversationId: String, withStatus status: ConversationStatus
    ) {
        let realm = LMDBManager.lmDBInstance()
        guard
            let conversationRO = ChatDBUtil.shared.getConversation(
                realm: realm, conversationId: conversationId)
        else { return }
    ...
    func savePostedConversation(
        savePostedConversationRequest: SavePostedConversationRequest
    ) {
        let conversation = savePostedConversationRequest.conversation
        let realm = LMDBManager.lmDBInstance()
        realm.writeAsync {
            guard
                let chatroomRO = ChatDBUtil.shared.getChatroom(
                    realm: realm, chatroomId: conversation.chatroomId),
                let conversationRO = ROConverter.convertConversation(
                    conversation: conversation)
            else { return }
            ...
            if chatroomRO.conversations.isEmpty == true {
                ...
                let tempConversationsRo = chatroomRO.conversations.where({
                    query in
            ...
            if (conversationRO.createdEpoch)
    ...
    func saveTemporaryConversation(request: SaveConversationRequest) {
        guard let conversation = request.conversation,
            let conversationRO = ROConverter.convertConversation(
                conversation: conversation)
        else { return }
        let realm = LMDBManager.lmDBInstance()
        guard
            let chatroomRO = ChatDBUtil.shared.getChatroom(
                realm: realm, chatroomId: conversationRO.chatroomId),
            let creatorRO = ROConverter.convertMember(
                member: conversation.member,
        ...
        LMDBManager.write(chatroomRO) { realm, object in
                    ...
                    ?? 0)
            {
                let lastConversationRO =
                    ROConverter.convertLastConversation(
                        realm: realm, conversation: conversation,
                        creator: creatorRO,
                        attachments: conversation.attachments,
                        deletedByMember: nil)
            ...
            if conversationRO.createdEpoch
                > (chatroomRO.lastSeenConversation?.createdEpoch
                    ?? 0)
            {
            ...
            if let convId = conversation.id,
                let deletedBy =
                    (conversation.deletedBy
                        ?? conversation.deletedByMember?.sdkClientInfo?.uuid),
                let ro = ChatDBUtil.shared.getConversation(
                    realm: realm, conversationId: convId),
    ...
    func updateEditedConversation(conversation: Conversation) {
        ...
        LMDBManager.update(conversationRO) { object in
            ...
            object.link = ROConverter.convertLink(
    ...
    func updateConversationReaction(reaction: String, conversationId: String) {
        ...
        LMDBManager.update(conversationRO) { object in
            guard
                let userUUID = realm.objects(UserRO.self).first?
                    .sdkClientInfoRO?.uuid
            else { return }
            let existingReactions = object.reactions.filter({
                $0.member?.sdkClientInfoRO?.uuid == userUUID
            })
            realm.delete(existingReactions)
            let member = ChatDBUtil.shared.getMember(
                realm: realm, communityId: object.communityId, uuid: userUUID)
    ...
    func getMemberBy(_ uuid: String) -> MemberRO? {
        ...
        guard
            let memberRO = ChatDBUtil.shared.getMember(
                realm: realm,
                communityId: SDKPreferences.shared.getCommunityId(), uuid: uuid)
        else { return nil }
        return memberRO
    }
    ...
    func updateLastConversationModel(
        chatroomId: String, conversation: Conversation
    ) {
        let realm = LMDBManager.lmDBInstance()
        ...
        LMDBManager.update(chatroomRO) { object in
            ...
            if chatroomRO.lastConversation?.id != conversationId {
                return
            }
            if let lastConversation = chatroomRO.lastConversation{
                lastConversation.deletedBy = conversationRO.deletedBy
                lastConversation.deletedByMember = conversationRO.deletedByMember
