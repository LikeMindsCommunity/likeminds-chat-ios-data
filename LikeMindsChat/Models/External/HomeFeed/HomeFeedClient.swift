...
class HomeFeedClient {
  ...
  func getExploreTabCount(
    withModuleName moduleName: String, response: LMClientResponse<GetExploreTabCountResponse>?
  ) {
    ...
    DataNetwork.shared.request(
      ...
      do {
        let result = try JSONDecoder().decode(
          LMResponse<GetExploreTabCountResponse>.self, from: data)
        response?(result)
      } catch let error {
        response?(LMResponse.failureResponse(error.localizedDescription))
      }
  ...
  func syncChatrooms() {
    ...
    if isFirstSync {
      SyncOperationUtil.startFirstHomeFeedSync(
        response: nil, chatroomTypes: [ChatroomType.normal.rawValue, ChatroomType.purpose.rawValue])
    } else {
      SyncOperationUtil.startReopenSyncForHomeFeed(
        response: nil, chatroomTypes: [ChatroomType.normal.rawValue, ChatroomType.purpose.rawValue])
    }
  }
  ...
  func getChatrooms(withObserver observer: HomeFeedClientObserver) {
    ...
    homeFeedGroupChatrooms = ChatDBUtil.shared.getChatrooms(
      realm: realmInstance, communityId: communityId)

    // Observe collection notifications. Keep a strong
    // reference to the notification token or the
    // observation will stop.
    chatroomResultnotificationToken = homeFeedGroupChatrooms?.observe {
      [weak self] (changes: RealmCollectionChange) in
      guard let self else { return }
      switch changes {
      case .initial(let collections):
        guard
          let chatrooms = homeFeedGroupChatrooms?.list.compactMap({ ro in
      ...
      case .update(let collections, let deletions, let insertions, let modifications):
        guard
          (homeFeedGroupChatrooms?.list.compactMap({ ro in
            return ModelConverter.shared.convertChatroomRO(chatroomRO: ro)
          })) != nil
        else {
          guard
        ...
        observers.forEach {
          $0?.onChange(
            removed: collections.compactMap({
              ModelConverter.shared.convertChatroomRO(chatroomRO: $0)
            }), inserted: self.getIndexedChatrooms(indexArray: insertions),
            updated: self.getIndexedChatrooms(indexArray: modifications))
        }
        break
  ...
  func observeLiveHomeFeed(forCommunity communityId: String) {
    firebaseRealTimeDBReference = FirebaseServiceConfiguration.getDatabaseReferenceForHomeFeed(
      communityId)
    firebaseRealTimeDBReference?.observe(
      .childChanged,
