...
class DirectMessageClient {
  ...
  */
  ...
  func syncDMChatrooms() {
    ...
    if isFirstSync {
      SyncOperationUtil.startFirstDMFeedSync(
        response: nil, chatroomTypes: [ChatroomType.directMessage.rawValue])
    } else {
      SyncOperationUtil.startReopenSyncForDMFeed(
        response: nil, chatroomTypes: [ChatroomType.directMessage.rawValue])
    }
  }
  ...
  func getChatrooms(withObserver observer: HomeFeedClientObserver) {
    ...
    feedDMChatrooms = ChatDBUtil.shared.getDMChatrooms(
    ...
    dmChatroomResultnotificationToken = feedDMChatrooms?.observe {
      [weak self] (changes: RealmCollectionChange) in
      guard let self else { return }
      switch changes {
      case .initial(let collections):
        guard
          let chatrooms = feedDMChatrooms?.list.compactMap({ ro in
      ...
      case .update(let collections, let deletions, let insertions, let modifications):
        guard
          (feedDMChatrooms?.list.compactMap({ ro in
            return ModelConverter.shared.convertChatroomRO(chatroomRO: ro)
          })) != nil
        else {
          guard
        ...
        dmObservers.forEach {
          $0?.onChange(
            ...
            }), inserted: self.getIndexedDMChatrooms(indexArray: insertions),
            updated: self.getIndexedDMChatrooms(indexArray: modifications))
        }
        break
  ...
  func checkDMStatus(
    request: CheckDMStatusRequest, response: LMClientResponse<CheckDMStatusResponse>?
  ) {
  ...
  func createDMChatroom(
    request: CreateDMChatroomRequest, response: LMClientResponse<CheckDMChatroomResponse>?
  ) {
