  ...
  enum NetworkPath {
    ...
    var apiURL: Endpoint {
      ...
      case .explorTabCount, .getExploreTabCount:
        return Endpoint(path: paths.communityMemberHomeMeta, queryItems: [])

      case .explorFeed(let request):
        var queryItems = [
          URLQueryItem(name: keys.page, value: "\(request.page)"),
          URLQueryItem(name: keys.orderType, value: "\(request.orderType)"),
        ]

        if let isPinned = request.isPinned {
          queryItems.append(URLQueryItem(name: keys.pinned, value: "\(isPinned)"))
        }

        return Endpoint(path: paths.communityFeed, queryItems: queryItems)
      ...
      case .checkDMStatus(let request):
        var queryItems = [
          URLQueryItem(name: keys.reqFrom, value: request.requestFrom?.rawValue ?? "")
        ]

        if let uuid = request.uuid {
          queryItems.append(URLQueryItem(name: keys.uuid, value: uuid))
        }

        if let chatroomId = request.chatroomId {
          queryItems.append(URLQueryItem(name: keys.chatroomId, value: chatroomId))
        }

        return Endpoint(path: paths.communityDmStatus, queryItems: queryItems)

      case .checkDMLimit(let request):
        let queryItems = [
          URLQueryItem(name: keys.uuid, value: request.uuid ?? "")
        ]
        return Endpoint(path: paths.chatroomDmLimit, queryItems: queryItems)
      ...
      case .syncChatrooms(let request):
        var queryItems = [
         URLQueryItem(name: keys.isLocalDb, value: "\(request.isLocalDB)"),
          URLQueryItem(name: keys.page, value: "\(request.page)"),
          URLQueryItem(name: keys.pageSize, value: "\(request.pageSize)"),
          URLQueryItem(name: keys.minTimestamp, value: "\(request.minTimestamp)"),
          URLQueryItem(name: keys.maxTimestamp, value: "\(request.maxTimestamp)"),
        ]
        ...
        if !request.chatroomTypes.isEmpty {
          queryItems.append(
            URLQueryItem(
              ...
              value: "[\(request.chatroomTypes.map({"\($0)"}).joined(separator: ","))]"
            ))
        }

        return Endpoint(path: paths.chatroomSync, queryItems: queryItems)

      case .getChatroomActions(let request):
        let queryItems = [
          URLQueryItem(name: keys.chatroomId, value: request.chatroomId)
        ]
      ...
      case .leaveSecretChatroom:
        return Endpoint(path: paths.chatroomParticipants, queryItems: [])
      ...
      case .getParticipants(let request):
        var queryItems = [
          URLQueryItem(name: keys.chatroomId, value: request.chatroomId),
          URLQueryItem(name: keys.isSecret, value: "\(request.isChatroomSecret)"),
          URLQueryItem(name: keys.page, value: "\(request.page)"),
          URLQueryItem(name: keys.pageSize, value: "\(request.pageSize)"),
        ]
        ...
        {
          queryItems.append(URLQueryItem(name: keys.participantName, value: searchName))
        }

        return Endpoint(path: paths.chatroomParticipants, queryItems: queryItems)
      ...
      case .getContentDownloadSettings:
        return Endpoint(path: paths.communitySettingsContentDownload, queryItems: [])
      ...
      case .getAllMembers(let request):
        ...
        if let memberState = request.memberState {
          queryItems.append(URLQueryItem(name: keys.memberState, value: "\(memberState)"))
        }
        ...
        if !request.filterMemberRoles.isEmpty {
          queryItems.append(
            URLQueryItem(
              ...
              value: "[\(request.filterMemberRoles.joined(separator: ","))]"
            ))
        }

        if let excludeSelf = request.excludeSelfUser {
          queryItems.append(URLQueryItem(name: keys.excludeSelfUser, value: "\(excludeSelf)"))
        }
      ...
      case .searchMembers(let request):
        ...
        if let memberState = request.memberState {
          queryItems.append(URLQueryItem(name: keys.memberStates, value: "\(memberState)"))
        }
      ...
      case .postConversation, .editConversation, .deleteConversations, .postPollConversation:
      ...
      case .syncConversations(let request):
        var queryItems = [
          ...
          URLQueryItem(name: keys.page, value: "\(request.page)"),
          URLQueryItem(name: keys.pageSize, value: "\(request.pageSize)"),
          URLQueryItem(name: keys.minTimestamp, value: "\(request.minTimestamp ?? 0)"),
          URLQueryItem(name: keys.maxTimestamp, value: "\(request.maxTimestamp ?? 0)"),
        ]

        if let conversationId = request.conversationId {
          queryItems.append(URLQueryItem(name: keys.conversationId, value: conversationId))
        }

        return Endpoint(path: paths.conversationSync, queryItems: queryItems)

      case .putReaction, .deleteReaction:
        return Endpoint(path: paths.conversationReaction, queryItems: [])
      ...
      case .getTaggingList(let request):
        ...
        if let searchName = request.searchName, !searchName.isEmpty {
          queryItems.append(URLQueryItem(name: keys.searchName, value: searchName))
        }

        if !request.chatroomId.isEmpty {
          queryItems.append(URLQueryItem(name: keys.chatroomId, value: request.chatroomId))
        }
      ...
      case .submitPoll:
        return Endpoint(path: paths.conversationPollSubmit, queryItems: [])

      case .getPollUsers(let request):
        let queryItems = [
          URLQueryItem(name: keys.conversationId, value: request.conversationId),
          URLQueryItem(name: keys.pollId, value: request.pollOptionId),
        ]
        return Endpoint(path: paths.conversationPollUsers, queryItems: queryItems)

      // MARK: Search Chatroom
      case .searchChatroom(let request):
        let queryItems = [
          URLQueryItem(name: keys.followStatus, value: "\(request.followStatus)"),
          URLQueryItem(name: keys.page, value: "\(request.page)"),
          URLQueryItem(name: keys.pageSize, value: "\(request.pageSize)"),
          URLQueryItem(name: keys.search, value: request.search),
          URLQueryItem(name: keys.searchType, value: request.searchType),
        ]
        return Endpoint(path: paths.chatroomSearch, queryItems: queryItems)
      ...
      case .searchConversation(let request):
        ...
        return Endpoint(path: paths.conversationSearch, queryItems: queryItems)
      ...
      case .getChannelInvites(let request):
        let queryItems = [
          URLQueryItem(name: keys.channelType, value: "\(request.channelType)"),
          URLQueryItem(name: keys.page, value: "\(request.page)"),
          URLQueryItem(name: keys.pageSize, value: "\(request.pageSize)"),
        ]
        return Endpoint(path: paths.channelInvites, queryItems: queryItems)

      case .updateChannelInvite(let request):
        let queryItems = [
          URLQueryItem(name: keys.channelId, value: "\(request.channelId)"),
          URLQueryItem(name: keys.inviteStatus, value: "\(request.inviteStatus.rawValue)"),
        ]
        return Endpoint(path: paths.channelInvite, queryItems: queryItems)

      //MARK: AIChatBots
      case .getAIChatbots(let request):
        let queryItems = [
            URLQueryItem(name: keys.page, value: "\(request.page ?? 1)"),
          URLQueryItem(name: keys.pageSize, value: "\(request.pageSize)"),
        ]
        return Endpoint(path: paths.communityChatbot, queryItems: queryItems)
      }
    }
